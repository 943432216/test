...
check_permit('info2_category', 'info2.category.mod');
...
if($_POST){
	...
	if(get_cfg('info2.category.upload_pic')){
		...
		if($BigPicPath=up_file($_FILES['PicPath'], $save_dir)){
			...
			$SmallPicPath=img_resize($BigPicPath, '', get_cfg('info2.category.pic_width'), get_cfg('info2.category.pic_height'));
			del_file($S_PicPath);
	...
	if($UnderTheCateId==0){
		...
		$UId=get_UId_by_CateId($UnderTheCateId, 'info2_category');
		$Dept=substr_count($UId, ',');
	}
	
	$SubUId=get_UId_by_CateId($CateId, 'info2_category');	//下级类别的UId
	$CurUId=$db->get_value('info2_category', "CateId='$CateId'", 'UId');
	$db->query("update info2_category set UId=replace(UId, '$CurUId', '$UId') where UId like '{$SubUId}%'");
	...
	if(get_cfg('info2.category.description')){
		$Description=save_remote_img($_POST['Description'], $save_dir);
		
		$db->update('info2_category_description', "CateId='$CateId'", array(
	...
	if(count(get_cfg('ly200.lang_array'))>1){
		add_lang_field('info2_category', array('Category', 'SeoTitle', 'SeoKeywords', 'SeoDescription'));
		add_lang_field('info2_category_description', 'Description');
	...
	set_page_url('info2_category', "CateId='$CateId'", get_cfg('info2.category.page_url'), 0);
...
$category_row=$db->get_one('info2_category', "CateId='$CateId'");
$category_description_row=$db->get_one('info2_category_description', "CateId='$CateId'");

include('../../inc/manage/header.php');
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('info2.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.mod');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php }?>
	<?php
	if(get_cfg('info2.category.dept')>1){
		$o_dept=$category_row['Dept']-($db->get_max('info2_category', "UId like '{$category_row['UId']}{$category_row['CateId']},%'", 'Dept')-get_cfg('info2.category.dept'));	//剩余可用的级数
		$e_where="CateId!='{$category_row['CateId']}' and Dept<".($category_row['SubCate']?$o_dept:get_cfg('info2.category.dept'));
	?>
		<tr>
			<td nowrap><?=get_lang('ly200.under_the');?>:</td>
			<td><?=ouput_Category_to_Select('UnderTheCateId', get_CateId_by_UId($category_row['UId']), 'info2_category', "UId='0,' and $e_where", $e_where, get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
	<?php if(get_cfg('info2.category.seo_tkd')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
		...
		<?php }?>
	<?php }?>
	<?php if(get_cfg('info2.category.description') && $CateId==8){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
