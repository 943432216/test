...
check_permit('info2_category', 'info2.category.add');
...
if($_POST){
	...
	$db->insert('info2_category', array(
			'UId'			=>	$UId,
			'Category'		=>	$Category,
	...
	get_cfg('info2.category.description') && $Description=save_remote_img($_POST['Description'], $save_dir);
	$db->insert('info2_category_description', array(
			'CateId'		=>	$CateId,
	...
	$db->update('manage_operation_log', 'Operation="info2_category_add"', array(
...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="category.php"><?=get_lang('info2.category_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.add');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php }?>
	<?php if(get_cfg('info2.category.dept')>1){?>
		<tr>
			<td nowrap><?=get_lang('ly200.under_the');?>:</td>
			<td><?=ouput_Category_to_Select('UnderTheCateId', $db->get_value('manage_operation_log', 'Operation="info2_category_add"', 'Value'), 'info2_category', 'UId="0,"', 'Dept<'.get_cfg('info2.category.dept'), get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
