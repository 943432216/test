...
if($_POST){
	...
	$db->insert('info2_contents', array(
			'InfoId'	=>	$InfoId,
			'Contents'	=>	$Contents
		)
	);
	...
	$db->update('manage_operation_log', 'Operation="info2_add"', array(
			'Value'	=>	$CateId
		)
	);
...
?>
<div class="header"><?=get_lang('ly200.current_location');?>:<a href="index.php"><?=get_lang('info2.info2_manage');?></a>&nbsp;-&gt;&nbsp;<?=get_lang('ly200.add');?></div>
...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php if($db->get_row_count('info2_category')>1){?>
		<tr>
			...
			<td><?=ouput_Category_to_Select('CateId', $db->get_value('manage_operation_log', 'Operation="info2_add"', 'Value'), 'info2_category', 'UId="0,"', 1, get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
	...
	<?php if(get_cfg('info2.is_in_index') || get_cfg('info2.is_hot')){?>
		<tr>
			...
			<td>
				<?php if(get_cfg('info2.is_in_index')){?><input name="IsInIndex" type="checkbox" value="1"><?=get_lang('ly200.is_in_index');?><?php }?>
				<?php if(get_cfg('info2.is_hot')){?><input name="IsHot" type="checkbox" value="1"><?=get_lang('info2.is_hot');?><?php }?>
			</td>
		</tr>
	<?php }?>
	<tr>
