...
<table width="100%" border="0" cellpadding="0" cellspacing="1" id="mouse_trBgcolor_table">
	...
	<?php if($db->get_row_count('product_category')>1){?>
		<tr>
			...
			<td><?=ouput_Category_to_Select('CateId', $db->get_value('manage_operation_log', 'Operation="product_add"', 'Value'), 'product_category', 'UId="0,"', 1, get_lang('ly200.select'));?></td>
		</tr>
	<?php }?>
	...
	<?php if(get_cfg('product.start_from')){?>
		<tr>
			...
			<td><input name="StartFrom" type="text" value="" class="form_input" size="5" maxlength="10" onkeyup="set_number(this, 0);" onpaste="set_number(this, 0);"></td>
		</tr>
	<?php }?>
	...
	<?php if(get_cfg('product.brief_description')){?>
		<?php for($i=0; $i<count(get_cfg('ly200.lang_array')); $i++){?>
			<tr>
				...
				<td><textarea name="BriefDescription<?=lang_name($i, 1);?>" rows="5" cols="60" class="form_area"></textarea></td>
			</tr>
		<?php }?>
	<?php }?>
